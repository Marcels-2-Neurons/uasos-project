...
# The run cycle should be implemented inside SRCevents.py
...
def drw_matrix(n=n_num, m=m_num):
    from settings import set
    h_space = (1 / 30)
    w_space = h_space / set.ratio[1]
    h_rect = 2 * ((1 - 4 * h_space) / n)
    w_rect = h_rect / set.ratio[1]
    ...
    for i in range(0, n):
        for j in range(0, m):
            xr[k] = (-1 + b_space + w_rect / 2) + j * (w_rect + w_space) - (
                        w_rect + w_space)  # Last term is used for giving space to flight Director
