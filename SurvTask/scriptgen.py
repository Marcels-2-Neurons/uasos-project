                    ...
                    for j in range(0, 9):
                        ...
                        if dice:  # Case TRUE: I have to add a target
                            ...
                            while not check:  # Recursive loop until target
                                ...
                                done = self.check_correct(iter=i, idx=j, num=self.IMGS[i][j], mode=self.TASK[i]) if self.TASK[i] in [1, 4] \
                                ...
                                if done in [1, 2, 3] and len(self.IMGS[i]) == len(set(self.IMGS[i])):  # Case Targets
                                    if done == 1:
                                        ...
                                        if aval_img[0] > 0 and aval_img[1] > 0:
                                            ...
                                            del self.IMGS[i][j]
                                            check = False
                                elif done in [4, 5, 6] or len(self.IMGS[i]) != len(set(self.IMGS[i])):  # Case Non-Target
                                    del self.IMGS[i][j]
                                    check = False
                            ...
                            while not check:  # Recursive loop until target
                                ...
                                if done in [1, 2, 3] and len(self.IMGS[i]) == len(set(self.IMGS[i])):  # Case Targets
                                    if done == 1:
                                        if aval_img[0] > 0:
                                            aval_img[0] -= 1
                                            del vec_idx[vec_idx.index(idx)] # remove the changed index from the candidates
                                            check = True
                                        else:
    ...
    def check_correct(self, iter=-1, idx=0, num=0, mode=0):  # This time the check is per-value, for cleariness purpose
        ...
        if digit_chk in NORBd.get_cat(num):
            ...
            if mode == 1 and self.find('1', NORBd.get_cat(num)) == 1:  # Case people
                ...
                self.look_tgts(iter)
                return 2
            elif (mode == 4 or mode == 1) and (self.find('1', NORBd.get_cat(num)) + self.find('4', NORBd.get_cat(num))) == 2:  # Case both
                self.look_tgts(iter)
                return 3
        else:
